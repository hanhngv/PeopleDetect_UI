...
MOVING_DIRECT MTrackObject::updateCounting(){
	int cur_bar = MPeopleCounting::STA_bar * MPeopleCounting::STA_resize_scale;
	int cur_avg_size = MPeopleCounting::STA_average_size * MPeopleCounting::STA_resize_scale;

	MOVING_DIRECT new_count = MOVING_DIRECT::UNDENTIFINED;

	list<MObject>::iterator last_state = m_object_hist.end();
	last_state--;

	if((abs((*last_state).m_center.y - cur_bar) > cur_avg_size / 2)
		&&(((*last_state).m_center.y - cur_bar) * (m_last_count_state.m_center.y - cur_bar) < 0)){
			if((*last_state).m_center.y > m_last_count_state.m_center.y)
				new_count = MOVING_DIRECT::ENTER;
			else
				new_count = MOVING_DIRECT::LEAVE;	

			m_last_count_state = *last_state;
	}

	return new_count;
}

MOVING_DIRECT MTrackObject::updateLastCounting(){
	int cur_bar = MPeopleCounting::STA_bar * MPeopleCounting::STA_resize_scale;
	int cur_avg_size = MPeopleCounting::STA_average_size * MPeopleCounting::STA_resize_scale;
	...
	if((*last_state).m_center == m_last_count_state.m_center)
		return new_count;

	if((abs((*last_state).m_center.y - m_last_count_state.m_center.y) > cur_avg_size / 2)
		&&(((*last_state).m_center.y - cur_bar) * (m_last_count_state.m_center.y - cur_bar) < 0)){
			if((*last_state).m_center.y > m_last_count_state.m_center.y)
				new_count = MOVING_DIRECT::ENTER;
			else
	...
	while (it != m_track_objs.begin())
	{
		...
		if((*it)->m_is_active){
			if(m_cur_update_time - (*it)->m_last_active > 0){
				...
				MOVING_DIRECT new_count = (*it)->updateLastCounting();
