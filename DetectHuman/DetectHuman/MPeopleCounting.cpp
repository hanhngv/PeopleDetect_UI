...
void MPeopleCounting::illu(Mat& img){
	...
	while(it != m_track_objs.begin()){
		...
		if((*it)->m_is_active
			&& (last.m_radius < 2 * STA_average_size * STA_resize_scale))
	...
	CSingleLock lock(&m_sync_illu_frame);
	while(lock.IsLocked())
		Sleep(1);
	lock.Lock();

	if(lock.IsLocked()){
		m_illu_frame = img.clone();
	}
...
float MPeopleCounting::distance(Point2f a, Point2f b){
	...
	return pow((double)tmp, 0.5);
}

void MPeopleCounting::getIlluFrame(Mat& illu_frame){
	CSingleLock lock(&m_sync_illu_frame);
	while(lock.IsLocked())
		Sleep(1);
	lock.Lock();
	...
	if(lock.IsLocked()){
		illu_frame = m_illu_frame.clone();
	}

	lock.Unlock();
}

void MPeopleCounting::getCountResult(int& num_enter, int& num_leave){
	CSingleLock lock(&m_sync_illu_frame);
	lock.Lock();

	if(lock.IsLocked()){
		num_enter = m_num_enter;
		num_leave = m_num_leave;
	}
