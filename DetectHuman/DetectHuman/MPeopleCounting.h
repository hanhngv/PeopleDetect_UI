...
public:
	...
	MOVING_DIRECT updateCounting();
	MOVING_DIRECT updateLastCounting();
...
class MPeopleCounting{
	...
	CCriticalSection m_sync_illu_frame;
	CCriticalSection m_sync_count;
...
public:
	...
	void addFrame();
	void processFrame();
	void getIlluFrame(Mat& illu_frame);
	void getCountResult(int& num_enter, int& num_leave);
};
